...
def menu_add(request):
    menu_form = MenuCreateForm()  # object banko
    context = {
        "form": menu_form,
        "title": "Menu Create Form"
    }

    if request.method == "POST":  # objext banyera call graya ani save garay
        ...
        # method two: argumented constructor menu_title = request.POST.get('menu_title') menu_desc =
        # request.POST.get('menu_desc') menu_ingredient = request.POST.get('menu_ingredient') menu_price =
        # request.POST.get('menu_price') data = Menu(menu_title=menu_title, menu_desc=menu_desc,
        # menu_ingredient=menu_ingredient, menu_price=menu_price) data.save()
...
def category_create(request):
    ...
    context = {
        'form': form,
        "title": "Category Create Form"  # yo tite add_category.html and create.htmlko ko ma xa
    }
